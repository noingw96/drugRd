...
def saveMoney():
    ...
    sheet = book.add_sheet('money')
    head = ['drugName','market','guige','price']  # 表头
    num=1
    ...
    for key in yaocaiName:
        try:
            ...
            mainContent = soup.find_all(class_="tableBase")[0]
            # img = mainContent.find_all(class_="picImg")[0].get('src')
            tbody = mainContent.find_all("tbody")[0]
            trs = tbody.find_all("tr")
            for tr in trs:
                guige = tr.find_all("td")[0].text
                market = tr.find_all("td")[1].text
                price = tr.find_all("td")[2].text[1:]
                sheet.write(num, 0, key)
                sheet.write(num, 1, market)
                sheet.write(num, 2, guige)
                sheet.write(num, 3, price)
                num+=1
        except:
            sheet.write(num, 0, key)
